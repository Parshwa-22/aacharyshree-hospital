        ...
        {!monks.length ? <p className="rounded-2xl border border-dashed border-slate-300 bg-white p-16 text-center text-slate-500">No monk locations published yet.</p> : <>
          ...
          {active && <section className="mt-8 overflow-hidden rounded-3xl bg-white shadow-xl">
            <div className="grid lg:grid-cols-[1.1fr_.9fr]">
              ...
              <div className="p-6 sm:p-8">{(active.photo || active.image) && <div className="relative mb-5 h-48 w-full overflow-hidden rounded-2xl bg-[#E8F8FC] shadow-sm"><img src={mediaUrl(active.photo || active.image)} alt="" aria-hidden="true" className="absolute inset-0 h-full w-full scale-110 object-cover object-center blur-xl" /><img src={mediaUrl(active.photo || active.image)} alt={active.name} className="relative h-full w-full object-contain object-center" /></div>}<div className="flex items-start justify-between gap-4"><div><h2 className="text-2xl font-bold text-[#0f2742]">{active.name}</h2>{active.travelReason && <p className="mt-2 text-slate-600">{active.travelReason}</p>}</div>{mapUrl(active) && <a href={mapUrl(active)} target="_blank" rel="noreferrer" className="rounded-full bg-[#26AFDE] p-3 text-white" aria-label="Open directions"><Navigation size={18} /></a>}</div>
