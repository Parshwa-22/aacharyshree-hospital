...
function mapUrl(monk) {
  if (monk.locationLink) return monk.locationLink;
  if (monk.latitude == null || monk.longitude == null) return "";
  return `https://www.google.com/maps?q=${monk.latitude},${monk.longitude}`;
}
        ...
        {!monks.length ? <p className="rounded-2xl border border-dashed border-slate-300 bg-white p-16 text-center text-slate-500">No monk locations published yet.</p> : <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {monks.map((monk) => <button key={monk.id} onClick={() => setSelected(monk)} className={`min-w-0 rounded-2xl border bg-white p-4 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-lg ${active?.id === monk.id ? "border-[#26AFDE] ring-2 ring-[#26AFDE]/20" : "border-slate-200"}`}>
              <div className="flex items-center gap-3"><div className="h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-[#E8F8FC]">{monk.photo ? <img src={monk.photo} alt={monk.name} className="h-full w-full object-cover" /> : <MapPin className="m-5 text-[#26AFDE]" />}</div><div className="min-w-0"><h2 className="truncate font-bold text-[#0f2742]">{monk.name}</h2>{monk.groupName && <p className="truncate text-sm text-slate-500">{monk.groupName}</p>}<p className="mt-1 text-xs text-[#1597C2]">{monk.locationLabel || "Location available"}</p></div></div>
            </button>)}
          </div>
          {active && <section className="mt-8 overflow-hidden rounded-3xl bg-white shadow-xl">
            <div className="grid lg:grid-cols-[1.1fr_.9fr]">
              <div className="min-h-[360px] bg-slate-100"><iframe title={`${active.name} current location`} src={`${mapUrl(active)}&output=embed`} className="h-full min-h-[360px] w-full border-0" loading="lazy" /></div>
